...
<p align=center><i>Some pieces of our model pipeline. Build featuresets of millions of datapoints (top), train hundreds of models at once and (bottom left) digest the results quickly so you can (bottom right) zero in on promising individual models.</i></p>
...
Atlas is a model development workflow built on Tensorflow and TFLearn to allow individual model developers to iterate quickly. Atlas consists of roughly four modules:
...
1. Create a featureset you wish to train models against.
    - The [feature library](ml/data/feature.py) has dozens of built-in features which can be generated from the GDS database. Each of these features can be built/referenced using a semantic language. The following creates a feature for the one-minute future log-returns on the bitstamp btc_usd pair.
      ```python
...
2. Create a [WorkUnit](ml/infra/work_unit.py) to define the model types and hyperparameters you wish to train.
    - A [ModelSpec](ml/infra/model_spec.py) is a description of model we would like to train: it's model class and any necessary hyperparameters to define it.
    - A [WorkUnit](ml/infra/work_unit.py) is a grouping of multiple ModelSpec's with a single featureset. When we run a workunit, we'll train all of these different models on the same featureset to see which of them is most suited to the prediction task.
3. Combine many WorkUnits into a single [WorkSpec](ml/infra/work_spec.py). WorkSpec's combine a list of work units together into a single batched run, and tell Atlas how to split the work between many GPUs.
...
7. Use batch overview visualizations to find the best models from your training run.
    - You can load the results object from the analysis console as follows:
...
1. Use individual model baseball cards and other visualizations to evaluate what worked and what didn't. 
...
1. Run production models with *model_runner*, and write Gryphon strategies that can read their outputs through redis.
  ```shell
